...
// ── SOURCES ──────────────────────────────────────────────────
// Each source builds its own URL given { type, tmdbId, season, episode }
const SOURCES = [
  {
    id:    'vidsrcme',
    label: 'VidSrc',
    movie: ({ tmdbId })           => `https://vidsrcme.ru/embed/movie?tmdb=${tmdbId}`,
    tv:    ({ tmdbId, season, episode }) =>
      `https://vidsrcme.ru/embed/tv?tmdb=${tmdbId}&season=${season}&episode=${episode}`,
    // iframe attrs this source needs
    ref:   'origin',
  },
  {
    id:    'vidsrcto',
    ...
    ref:   'no-referrer-when-downgrade',
  },
  {
    id:    'vidlink',
    label: 'VidLink',
...
// active source index, persisted across episodes
let activeSource = 0;

function getEmbedUrl(sourceIdx) {
  const src = SOURCES[sourceIdx];
  if (S.playerType === 'movie') {
    return src.movie({ tmdbId: S.playerItem?.id });
  } else {
    return src.tv({ tmdbId: S.playerItem?.id, season: S.playerSeason, episode: S.playerEpisode });
  }
}
...
function switchSource(idx) {
  activeSource = idx;
  // just swap the iframe src + update buttons, no full re-render
  const iframe = document.getElementById('playerIframe');
  if (iframe) {
    iframe.src = getEmbedUrl(idx);
  }
...
function renderPlayer() {
  ...
  const item    = S.playerItem;
  const isTV    = S.playerType === 'tv';
  const eps     = S.playerEps;
  const season  = S.playerSeason;
  const episode = S.playerEpisode;
  const seriesId = item?.id;

  const title = isTV
    ? `${esc(item?.name || item?.title || '')} — S${pad(season)} E${pad(episode)}`
  ...
  const nav = isTV ? `
    <div class="player-nav">
      ...
      <span class="player-ep-label">${title}</span>
      <button ${next ? '' : 'disabled'}
        onclick="playEpisode(${seriesId},${season},${next ? eps[idx+1].episode_number : 0})">
        Next →
      </button>
    </div>` :
    `<div class="player-ep-label" style="margin-top:10px;text-align:center">${title}</div>`;

  // source switcher buttons
  const srcBtns = SOURCES.map((s, i) =>
    `<button class="src-btn${i === activeSource ? ' active' : ''}"
      onclick="switchSource(${i})">${s.label}</button>`
  ).join('');

  const embedUrl = getEmbedUrl(activeSource);
  const ref      = SOURCES[activeSource].ref;

  document.getElementById('playerView').innerHTML = `
    <div class="player-wrap">
      <div class="player-top-bar">
        <div class="player-back" onclick="goBackToDetail()">← Back</div>
...
// ── POPUP BLOCKER ─────────────────────────────────────────────
// Primary defence: sandbox="allow-scripts allow-same-origin" on the iframe
// blocks window.open, target=_blank, and top-frame navigation natively.
// Secondary: override window.open in our page context as belt-and-suspenders.
let _popupBlockerInstalled = false;
function installPopupBlocker() {
  if (_popupBlockerInstalled) return;
  _popupBlockerInstalled = true;

  // Kill any window.open call that bubbles up
  window.open = () => null;

  // If the iframe somehow blurs the window (redirect attempt), snap focus back
